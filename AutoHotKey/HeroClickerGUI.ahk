...
;; README ;;
;;;;;;;;;;;;
; Elige una velocidad y pulsa "Start". De vez en cuando tira el combo.
; Parámetros disponibles:
;	--detach: Despega la ventana del clicker de la ventana de "Clicker Heroes"

;;;;;;;;;;;;;;;;;;
;; DOCK LIBRARY ;;
;;;;;;;;;;;;;;;;;;

; Original library: https://autohotkey.com/boards/viewtopic.php?f=6&t=30912
	...
	/*
		Instance.Position(pos)
			pos - sets position to dock client window
				...
				L or Left -  left side
				Custom X Y - Left side, X & Y Offset
	*/
	Position(pos)
	{
		this.pos := pos
	...
	class EventsHandler extends Dock.HelperFunc
	{
		...
		*/
		EVENT_OBJECT_LOCATIONCHANGE(self, via)
		{
			...
			{
				; TODO: A veces se chinga y se esconde detras.
				vPos := self.pos
				pos_array := StrSplit(vPos, A_Space)
				vMyX := pos_array[2]
				vMyY := pos_array[3]
				If (via = "host")
				{
					Return this.MoveWindow(self.hwnd.Client	 	;hwnd
								, Host.x - Client.w + vMyX	;x
								, Host.y + vMyY		;y
								, Client.w	  	;width
								, Client.h)	 	;height	
				}
...
; --- Variables
speed=50 ; timer's ms
started=0 ; timer/s status (0=false, 1=true)
pDetach := false
...
; --- GUI
Gui, +hwndGuihwnd
Gui, Add, Radio, y15 vOptRadio, 20ms
Gui, Add, Radio, y+10 Checked, 50ms

Gui, Add, Button, w100 y10 r3 gButtonStartStop vButtonText, Start
Gui, Add, Button, xm+0 y+10 w157 gCombo vButtonCombo, Combo

Gui, Show

if(!pDetach) {
	
	exDock := new Dock(WinExist("Clicker Heroes"), Guihwnd)
	;exDock.Position("Custom 100 100") ; TODO: A veces se chinga y se esconde detras.
	exDock.Position("L")
	exDock.CloseCallback := Func("CloseCallback")
} else {
	Host := Dock.HelperFunc.WinGetPos(WinExist("Clicker Heroes"))
	Client := Dock.HelperFunc.WinGetPos(Guihwnd)
	
	ShowX := Host.x - Client.w ;x
	ShowY := Host.y ;x
	Gui, Show, x%ShowX% y%ShowY%
}
Return
...
ButtonStartStop:
	...
	; Start or stop timer and change button text
	if(started) {
		settimer, autoclick, Off ; Turns off the autoclicking
		GuiControl ,, ButtonText, Start
	} else {
		settimer, autoclick, %speed% ; clicks every [n]ms
...
Combo:
	ControlSend ,, 8, Clicker Heroes ; Double effect
	ControlSend ,, 7, Clicker Heroes ; +200% Damage per click
	ControlSend ,, 3, Clicker Heroes ; +50% Crit chance
	ControlSend ,, 2, Clicker Heroes ; +100% DPS
	ControlSend ,, 1, Clicker Heroes ; +10 clicks/s
	ControlSend ,, 4, Clicker Heroes ; +100% Gold
	ControlSend ,, 5, Clicker Heroes ; Gold for each click
	; Once this combo finishes, use 9 to reload 5 and get even more gold
Return
...
; --- Timer
autoclick:
	ControlClick,,Clicker Heroes,,,, NA x850 y350  ; Clicks in NA mode at coordinates that are relative to a named control.
Return
